...
public class UsuarioDAO {
    ...
    public void insertarUsuario(Usuario usuario) throws SQLException {
        ...
        if (!objMySQLConnection.isError()) {
            String sql = String.format(
                    "INSERT INTO usuario (nombre, apellidos, username, email, password, telefono, id_rol,id_direccion, activo, fecha_registro) "
                            +
                            "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %d, %d, %b, '%s')",
                    usuario.getNombre(), usuario.getApellidos(), usuario.getUsername(), usuario.getEmail(),
                    usuario.getPassword(), usuario.getTelefono(), usuario.getIdRol(), usuario.getIdDireccion(),
                    usuario.isActivo(),
    ...
    public Usuario obtenerUsuarioPorId(int idUsuario) {
        ...
        objMySQLConnection.open();
        if (!objMySQLConnection.isError()) {
            String query = "SELECT id_usuario, nombre, apellidos, username, email, password, telefono, id_rol, id_direccion, activo, fecha_registro FROM usuario WHERE id_usuario = '"
                + idUsuario + "'";
            ResultSet resultSet = objMySQLConnection.executeSelect(query);
            try {
                if (resultSet != null && resultSet.next()) {
